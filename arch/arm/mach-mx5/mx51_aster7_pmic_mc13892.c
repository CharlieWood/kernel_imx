/*
 * mx51-aster7-pmic-mc13892.c  --  i.MX51 Aster7 Driver for Atlas MC13892 PMIC
 */
 /*
...
/*	
 *	0x92412, set status of regulators to be on/off in standby mode
 *	VUSB2 OFF
 *	VIOHI ON
 *	VPLL ON
 *	VCAM RECOMMENT ON
 *	VGEN2 ON
 */
#define REG_MODE_0_ALL_MASK	(GEN1_STBY_MASK | USB2_STBY_MASK)

/*	
 *	0x92082 set status of regulators to be on/off in standby mode
 *	VVIDEO ON
 *	VAUDIO RECOMMENT ON
 */
#define REG_MODE_1_ALL_MASK	(SD_STBY_MASK)
...
static int mc13892_regulator_init(struct mc13892 *mc13892)
{
	...
	printk(KERN_INFO "Initializing regulators for Aster7.\n");
	...
	value &= ~(GEN1_STBY_MASK | IOHI_STBY_MASK | DIG_STBY_MASK | GEN2_STBY_MASK | PLL_STBY_MASK | USB2_STBY_MASK);
	value |= REG_MODE_0_ALL_MASK;
	pmic_write_reg(REG_MODE_0, value, 0xffffff);

	pmic_read_reg(REG_MODE_1, &value, 0xffffff);
	value &= ~(GEN3_STBY_MASK | CAM_STBY_MASK | VIDEO_STBY_MASK | AUDIO_STBY_MASK | SD_STBY_MASK);
	value |= REG_MODE_1_ALL_MASK;
	pmic_write_reg(REG_MODE_1, value, 0xffffff);

	/* set VAUDIO to be 2.775V */
	pmic_read_reg(REG_SETTING_1, &value, 3<<4);
	value = 2<<4;
	pmic_write_reg(REG_SETTING_1, value, 3<<4);
...
int __init mx51_aster7_init_mc13892(void)
{
