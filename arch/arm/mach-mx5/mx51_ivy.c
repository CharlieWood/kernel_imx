...
static iomux_v3_cfg_t mx51_ivy_pads[] = {
	/* charger ac_in */
...
{
	iomux_v3_cfg_t pad_gpio_out = MX51_PAD_GPIO1_2__GPIO1_2;
	iomux_v3_cfg_t pad_pwm1_out = MX51_PAD_GPIO1_2__PWM1_PWMO;
...
static struct i2c_board_info mxc_i2c0_board_info[] __initdata = {
	...
	//.irq  = gpio_to_irq(IVY_G_SENSOR_INT),
	},
	{	/* compass */
	.type = "hmc5883",
	.addr = 0x1e,
	.platform_data = &hmc5883_data,
	.irq  = gpio_to_irq(IVY_COMPASS_INT),
	}, 
...
static struct resource mx51_wifi_resources[] = {
	[0] = {
		...
		.start =  gpio_to_irq(IVY_WLAN_WAKEUP_HOST),
		.end   =  gpio_to_irq(IVY_WLAN_WAKEUP_HOST),
...
static struct mxc_audio_platform_data sgtl5000_data = {
	...
	.src_port = 2,
	.ext_port = 3,
	.hp_irq = gpio_to_irq(IVY_HPHONE_DET),
...
static int gps_set_power(int enable)
{
	...
	if (enable) {
		gpio_set_value(IVY_GPS_PWR_ON, 1);
	} else {
		gpio_set_value(IVY_GPS_PWR_ON, 0);
	}
	return 0;
}
...
static int gps_set_nrst(int nrst)
{
	...
	gpio_set_value(IVY_GPS_RST, nrst);
	return 0;
}
...
static struct resource mx51_bluesleep_resources[] = {
	...
	[2] = {
		.name  = "bt_wakeup_host",
		.start =  gpio_to_irq(IVY_BT_WAKEUP_HOST),
		.end   =  gpio_to_irq(IVY_BT_WAKEUP_HOST),
...
static void mxc_register_powerkey(pwrkey_callback pk_cb)
{
	...
	int irq, ret;
	irq = gpio_to_irq(IVY_SYS_ON_OFF_REQ);
...
static void __init mxc_board_init(void)
{
	...
	/* SD card detect irqs */
	mxcsdhc1_device.resource[2].start = gpio_to_irq(IVY_SD_MMC_SD);
	mxcsdhc1_device.resource[2].end = gpio_to_irq(IVY_SD_MMC_SD);
