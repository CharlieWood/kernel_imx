...
static DECLARE_WAIT_QUEUE_HEAD(bma150_waitq);
...
static irqreturn_t bma150_irq_handler(int irq, void *_id)
{
	...
	interrupt_flag = 1;
	wake_up_interruptible(&bma150_waitq); 
    if(data->async_queue)
...
static unsigned int bma150_poll(struct file *file, poll_table *wait)
{
	...
	poll_wait(file,&bma150_waitq,wait);
	wait_event_interruptible(bma150_waitq,interrupt_flag);
