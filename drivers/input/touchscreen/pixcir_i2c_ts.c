...
struct pixcir_touchkey *pixcir_touchkey_map = NULL;

static int pixcir_touchkey_report(struct input_dev *dev, int x_value, int y_value)
{
	int i = 0, dalt, key, bit, x_pos, y_pos;

	while (pixcir_touchkey_map) {
		key = pixcir_touchkey_map[i].touch_key;
		bit = pixcir_touchkey_map[i].bit;
		x_pos = pixcir_touchkey_map[i].x_pos;
		y_pos = pixcir_touchkey_map[i].y_pos;
		if (key < 0) {
			i = 0;
			break;
		}
		...
		if (dalt < 20) {
			...
			if (dalt < 20) {
				if (!(key_bit & bit)) {
					input_event(dev, EV_KEY, pixcir_touchkey_map[i].touch_key, 1);
					key_bit |= bit;
				}
				return 1;
			}
...
static void pixcir_touchkey_release(struct input_dev *dev)
{
	int i = 0, key, bit;
...
static void pixcir_ts_poscheck(struct work_struct *work)
{
	...
	if (!(touching)) {
		pixcir_touchkey_release(tsdata->input);
		z = 0;
		w = 0;
	}

	if (touching == 1) {
		if(!pixcir_touchkey_report(tsdata->input, posx1, posy1)) {
...
static unsigned char pixcir_get_fw_ver(struct i2c_client *client)
{
	int ret;
...
#if 0
	int ret;
	unsigned char freeze_buf[2];

	memset(freeze_buf, 0, sizeof(freeze_buf));
	freeze_buf[0] = POW_MOD_REG;
	freeze_buf[1] = FREEZE_MODE;

	/* I give freeze mode to panel, he will sleep */
	ret = i2c_master_send(client, freeze_buf, 2);
	if (ret < 0) {
		printk(KERN_ERR "%s: enter sleep mode fail\n", __func__);
		return ret;
	}
#endif
...
static int pixcir_i2c_ts_resume(struct i2c_client *client)
{
...
#if 0
	int ret;
	unsigned char active_buf[2];

	memset(active_buf, 0, sizeof(active_buf));
	active_buf[0] = POW_MOD_REG;
	active_buf[1] = ACTIVE_MODE;
	...
	/* I give active mode to panel, he will wake up */
	ret = i2c_master_send(client, active_buf, 2);
	if (ret < 0) {
		printk(KERN_ERR "%s: enter active mode fail\n", __func__);
		return ret;
	}
#endif
...
static void pixcir_early_suspend(struct early_suspend *earlysuspend)
{
	struct pixcir_i2c_ts_data *tsdata;
	tsdata = container_of(earlysuspend, struct pixcir_i2c_ts_data, early_suspend);

	if (pixcir_i2c_ts_suspend(tsdata->client, PMSG_SUSPEND) != 0)
		dev_err(&tsdata->client->dev, "%s: failed\n", __func__);
	printk(KERN_WARNING "touch suspend: %s %d\n", __FUNCTION__, __LINE__);
}

static void pixcir_late_resume(struct early_suspend *earlysuspend)
{
	struct pixcir_i2c_ts_data *tsdata;
	tsdata = container_of(earlysuspend, struct pixcir_i2c_ts_data, early_suspend);

	if (pixcir_i2c_ts_resume(tsdata->client) != 0)
		dev_err(&tsdata->client->dev, "%s: failed\n", __func__);
	printk(KERN_WARNING "touch resume: %s %d\n", __FUNCTION__, __LINE__);
}
#endif
...
{
	int i = 0, xmin, xmax, ymin, ymax, fw_ver;
	struct pixcir_i2c_ts_data *tsdata;
	struct pixcir_touch_pdata *pdata = NULL;
	struct pixcir_callback_pdata *ppdata;
	struct input_dev *input;
	struct device *dev;
	struct i2c_dev *i2c_dev;
	int error;
	...
	fw_ver = pixcir_get_fw_ver(client);
	if (ppdata->callback)
		pdata = ppdata->callback(fw_ver);
	else
		return -ENOMEM;
	...
	if (xmax <= 0 || ymax <= 0) {
		printk(KERN_ERR "%s, xmax or ymax is less than zero\n", __func__);
		error = -ENOMEM;
		return error;
	}

	gpio_int = pdata->gpio_int;
	gpio_reset = pdata->gpio_reset;

	if (pdata->pixcir_touchkey_map)
		pixcir_touchkey_map = pdata->pixcir_touchkey_map;
...
#ifdef TOUCH_KEY_ENABLE
	while (pixcir_touchkey_map) {
		if (pixcir_touchkey_map[i].touch_key < 0) {
			i = 0;
			break;
		}
		set_bit(pixcir_touchkey_map[i].touch_key, input->keybit);
		i++;
	}
#endif
	...
	input_set_abs_params(input, ABS_MT_POSITION_X, xmin, xmax, 0, 0);
	input_set_abs_params(input, ABS_MT_POSITION_Y, ymin, ymax, 0, 0);
	...
	tsdata->early_suspend.suspend = pixcir_early_suspend;
	tsdata->early_suspend.resume = pixcir_late_resume;
...
static long pixcir_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	#if PIXCIR_DEBUG
	printk("cmd = %d,arg = %d, gpio_reset:%d\n", cmd, arg, gpio_reset);
	#endif
...
static ssize_t pixcir_write(struct file *file, const char __user *buf,size_t count, loff_t *ppos)
{
	...
	tsdata = dev_get_drvdata(&client->dev);
	switch(status_reg)
	{
		case CALIBRATION_FLAG: //CALIBRATION_FLAG=1
...
{ 	.driver =
		{
			.owner = THIS_MODULE,
			.name = "pixcir_i2c_ts_driver_v2.4",
		}, 
