...
{
	if (fsize->index > ov2655_mode_MAX)
		return -EINVAL;

	fsize->pixel_format = ov2655_data.pix.pixelformat;
	fsize->discrete.width =
			max(ov2655_mode_info_data[0][fsize->index].width,
			    ov2655_mode_info_data[1][fsize->index].width);
	fsize->discrete.height =
			max(ov2655_mode_info_data[0][fsize->index].height,
			    ov2655_mode_info_data[1][fsize->index].height);
	return 0;
}
...
static int ioctl_g_chip_ident(struct v4l2_int_device *s, int *id)
{
	...
	strcpy(((struct v4l2_dbg_chip_ident *)id)->match.name, "ov2655_camera");
...
{
	if (fmt->index > ov2655_mode_MAX)
		return -EINVAL;

	fmt->pixelformat = ov2655_data.pix.pixelformat;
